...
class InventoryFetchController extends Controller
{
    ...
    public function fetchInactiveInventory(Request $request)
    {
        ...
        if (empty($request->input('search.value'))) {
            ...
            $posts = Inventory::onlyTrashed()->where('name', 'like', "%{$productName}%")
                ->where('quantity', 'like', "%{$productQuantity}%")
                ->where('quantity_in_use', 'like', "%{$productQuantityInUse}%")
                ->where('quantity_available', 'like', "%{$productQuantityAvailable}%")
                ->where('created_at', 'like', "%{$productCreatedAt}%")
            ...
            //total number of filtered data
            $totalFiltered = Inventory::onlyTrashed()->where('name', 'like', "%{$productName}%")
            ->where('quantity', 'like', "%{$productQuantity}%")
            ->where('quantity_in_use', 'like', "%{$productQuantityInUse}%")
            ->where('quantity_available', 'like', "%{$productQuantityAvailable}%")
            ->where('created_at', 'like', "%{$productCreatedAt}%")
            ...
            $posts = Inventory::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('quantity', 'like', "%{$search}%")
                ->orWhere('quantity_in_use', 'like', "%{$search}%")
                ->orWhere('quantity_available', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = Inventory::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('quantity', 'like', "%{$search}%")
                ->orWhere('quantity_in_use', 'like', "%{$search}%")
                ->orWhere('quantity_available', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
    ...
    public function fetchInactiveInventoryCategory(Request $request)
    {
        //column list in the table Prpducts
        $columns = array(
            0 => 'name',
        ...
        //get the total number of data in Category table
        $totalData = InventoryCategory::onlyTrashed()->count();
        //total number of data that will show in the datatable default 10
        $limit = $request->input('length');
        //start number for pagination ,default 0
        $start = $request->input('start');
        ...
        if (empty($request->input('search.value'))) {
            //get all the category data
            $posts = InventoryCategory::onlyTrashed()->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();

            //total number of filtered data
            $totalFiltered = InventoryCategory::onlyTrashed()->count();
        } else {
            $search = $request->input('search.value');

            $posts = InventoryCategory::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = InventoryCategory::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
                ->count();
        }
        ...
        if ($posts) {
            ...
            foreach ($posts as $r) {
                ...
                $nestedData['created_at'] = date('d-m-Y', strtotime($r->created_at));
                $nestedData['action'] = '
                    <button name="restore" id="restore-inventory-category" data-id="' . $r->id . '" class="btn btn-success btn-sm">Restore</button>
				';
                $data[] = $nestedData;
            }
        }
