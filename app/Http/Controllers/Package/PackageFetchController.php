...
class PackageFetchController extends Controller
{
    ...
    public function fetchInactivePackage(Request $request)
    {
        //column list in the table Prpducts
        $columns = array(
            0 => 'name',
        ...
        //get the total number of data in Category table
        $totalData = Package::onlyTrashed()->count();
        //total number of data that will show in the datatable default 10
        $limit = $request->input('length');
        //start number for pagination ,default 0
        $start = $request->input('start');
        ...
        if (empty($request->input('search.value'))) {
            ...
            //total number of filtered data
            $totalFiltered = Package::onlyTrashed()->count();
        } else {
            $search = $request->input('search.value');

            $posts = Package::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('package_pax', 'like', "%{$search}%")
                ->orWhere('package_price', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
                ->orWhereHas('main_package', function ($query) use ($search) {
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = Package::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('package_pax', 'like', "%{$search}%")
                ->orWhere('package_price', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
                ->orWhereHas('main_package', function ($query) use ($search) {
        ...
        if ($posts) {
            ...
            foreach ($posts as $r) {
                ...
                $nestedData['created_at'] = date('d-m-Y', strtotime($r->created_at));
                $nestedData['action'] = '
                    <button name="restore" id="restore-package" data-id="' . $r->id . '" class="btn btn-success btn-sm">Restore</button>
				';
                $data[] = $nestedData;
            }
        }
    ...
    public function fetchInactivePackageTask(Request $request)
    {
        //column list in the table Prpducts
        $columns = array(
            0 => 'name',
        ...
        //get the total number of data in Category table
        $totalData = PackageTask::onlyTrashed()->where('package_id', $request->package_id)->count();
        //total number of data that will show in the datatable default 10
        $limit = $request->input('length');
        //start number for pagination ,default 0
        $start = $request->input('start');
        ...
        if (empty($request->input('search.value'))) {
            //get all the category data
            $posts = PackageTask::onlyTrashed()->where('package_id', $request->package_id)
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            ...
            //total number of filtered data
            $totalFiltered = PackageTask::onlyTrashed()->where('package_id', $request->package_id)->count();
        } else {
            $search = $request->input('search.value');
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = PackageTask::onlyTrashed()->where('package_id', $request->package_id)
                ->where(function ($query) use ($search) {
                    $query->orWhere('name', 'like', "%{$search}%");
                })
                ->count();
        ...
        if ($posts) {
            ...
            foreach ($posts as $r) {
                ...
                if($request->is_show == 0){
                    $nestedData['action'] = '
                        <button name="restore" id="restore-package-task" data-id="' . $r->id . '" class="btn btn-success btn-sm">Restore</button>
                    ';
                }
                $data[] = $nestedData;
            }
    ...
    public function fetchInactivePackageFood(Request $request)
    {
        //column list in the table Prpducts
        $columns = array(
            0 => 'category_id',
        ...
        //get the total number of data in Category table
        $totalData = PackageMenu::onlyTrashed()->where('package_id', $request->package_id)->count();
        //total number of data that will show in the datatable default 10
        $limit = $request->input('length');
        //start number for pagination ,default 0
        $start = $request->input('start');
        ...
        if (empty($request->input('search.value'))) {
            //get all the category data
            $posts = PackageMenu::onlyTrashed()->where('package_id', $request->package_id)
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            ...
            //total number of filtered data
            $totalFiltered = PackageMenu::onlyTrashed()->where('package_id', $request->package_id)->count();
        } else {
            $search = $request->input('search.value');
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = PackageMenu::onlyTrashed()->where('package_id', $request->package_id)
                ->where(function ($query) use ($search) {
                    $query->orWhereHas('package_food_category', function ($query) use ($search) {
                        $query->where('name', 'like', "%{$search}%");
                    })
        ...
        if ($posts) {
            ...
            foreach ($posts as $r) {
                ...
                if($request->is_show == 0){
                    $nestedData['action'] = '
                        <button name="restore" id="restore-package-food" data-id="' . $r->id . '" class="btn btn-success btn-sm">Restore</button>
                    ';
                }
                $data[] = $nestedData;
            }
    ...
    public function fetchInactivePackageEquipment(Request $request)
    {
        //column list in the table Prpducts
        $columns = array(
            0 => 'inventory_id',
        ...
        //get the total number of data in Category table
        $totalData = PackageEquipments::onlyTrashed()->where('package_id', $request->package_id)->count();
        //total number of data that will show in the datatable default 10
        $limit = $request->input('length');
        //start number for pagination ,default 0
        $start = $request->input('start');
        ...
        if (empty($request->input('search.value'))) {
            //get all the category data
            $posts = PackageEquipments::onlyTrashed()->where('package_id', $request->package_id)
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            ...
            //total number of filtered data
            $totalFiltered = PackageEquipments::onlyTrashed()->where('package_id', $request->package_id)->count();
        } else {
            $search = $request->input('search.value');
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = PackageEquipments::onlyTrashed()->where('package_id', $request->package_id)
                ->where(function ($query) use ($search) {
                    $query ->orWhereHas('package_equipment', function ($query) use ($search) {
                        $query->where('name', 'like', "%{$search}%");
                    })
        ...
        if ($posts) {
            ...
            foreach ($posts as $r) {
                ...
                if($request->is_show == 0){
                    // <button name="edit" id="edit-equipment-package" data-quantity="' . $r->quantity . '"  data-inventory-id="' . $r->inventory_id . '" data-id="' . $r->id . '" class="btn btn-warning btn-xs">Edit</button>
                    $nestedData['action'] = '
                        <button name="restore" id="restore-package-equipment" data-id="' . $r->id . '" class="btn btn-success btn-sm">Restore</button>
                    ';
                }
                $data[] = $nestedData;
            }
    ...
    public function fetchInactivePackageOther(Request $request)
    {
        //column list in the table Prpducts
        $columns = array(
            0 => 'name',
        ...
        //get the total number of data in Category table
        $totalData = PackageOther::onlyTrashed()->where('package_id', $request->package_id)->count();
        //total number of data that will show in the datatable default 10
        $limit = $request->input('length');
        //start number for pagination ,default 0
        $start = $request->input('start');
        ...
        if (empty($request->input('search.value'))) {
            //get all the category data
            $posts = PackageOther::onlyTrashed()->where('package_id', $request->package_id)
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            ...
            //total number of filtered data
            $totalFiltered = PackageOther::onlyTrashed()->where('package_id', $request->package_id)->count();
        } else {
            $search = $request->input('search.value');
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = PackageOther::onlyTrashed()->where('package_id', $request->package_id)
                ->where(function ($query) use ($search) {
                    $query->orWhere('name', 'like', "%{$search}%")
                        ->orWhere('service_price', 'like', "%{$search}%");
                })
        ...
        if ($posts) {
            ...
            foreach ($posts as $r) {
                ...
                if($request->is_show == 0){
                    $nestedData['action'] = '
                        <button name="restore" id="restore-package-other" data-id="' . $r->id . '" class="btn btn-success btn-sm">Restore</button>
                    ';
                }
