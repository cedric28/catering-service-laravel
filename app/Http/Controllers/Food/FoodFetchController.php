...
class FoodFetchController extends Controller
{
    ...
    public function fetchInactiveFood(Request $request)
    {
        //column list in the table Prpducts
        $columns = array(
            0 => 'name',
        ...
        //get the total number of data in Category table
        $totalData = Foods::onlyTrashed()->count();
        //total number of data that will show in the datatable default 10
        $limit = $request->input('length');
        //start number for pagination ,default 0
        $start = $request->input('start');
        ...
        if (empty($request->input('search.value'))) {
            //get all the category data
            $posts = Foods::onlyTrashed()->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();

            //total number of filtered data
            $totalFiltered = Foods::onlyTrashed()->count();
        } else {
            $search = $request->input('search.value');

            $posts = Foods::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
                ->orWhereHas('category', function ($query) use ($search) {
                    $query->where('name', 'like', "%{$search}%");
                })
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = Foods::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
                ->orWhereHas('category', function ($query) use ($search) {
                    $query->where('name', 'like', "%{$search}%");
                })
        ...
        if ($posts) {
            ...
            foreach ($posts as $r) {
                ...
                $nestedData['created_at'] = date('d-m-Y', strtotime($r->created_at));
                $nestedData['action'] = '
                    <button name="restore" id="restore-food" data-id="' . $r->id . '" class="btn btn-success btn-sm">Restore</button>
				';
                $data[] = $nestedData;
            }
        }
    ...
    public function fetchInactiveFoodCategory(Request $request)
    {
        //column list in the table Prpducts
        $columns = array(
            0 => 'name',
        ...
        if (empty($request->input('search.value'))) {
            //get all the category data
            $posts = Category::onlyTrashed()->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            ...
            $posts = Category::onlyTrashed()->where('name', 'like', "%{$search}%")
                ->orWhere('created_at', 'like', "%{$search}%")
                ->orWhereHas('dish_category', function ($query) use ($search) {
                    $query->where('name', 'like', "%{$search}%");
                })
            ...
            //total number of filtered data matching the search value request in the Category table	
            $totalFiltered = Category::onlyTrashed()->where('name', 'like', "%{$search}%")
                    ->orWhere('created_at', 'like', "%{$search}%")
                    ->orWhereHas('dish_category', function ($query) use ($search) {
                        $query->where('name', 'like', "%{$search}%");
                    })
        ...
        if ($posts) {
            ...
            foreach ($posts as $r) {
                ...
                $nestedData['created_at'] = date('d-m-Y', strtotime($r->created_at));
                $nestedData['action'] = '
                    <button name="restore" id="restore-food-category" data-id="' . $r->id . '" class="btn btn-success btn-sm">Restore</button>
				';
                $data[] = $nestedData;
            }
        }
