	...
	<div class="page-header page-header-light">
		<div class="page-header-content header-elements-md-inline">
			<div class="page-title d-flex">
				<h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Inventories</span> - New Record</h4>
		...
		<div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
			<div class="d-flex">
				<div class="breadcrumb">
					...
					<a href="{{ route('inventories.create')}}" class="breadcrumb-item active"> Add New Record</a>
				</div>
				...
				<div class="col-md-10 offset-md-1">
					<form action="{{ route('inventories.store')}}" method="POST">
						...
						<div class="form-group row">
							...
							<div class="col-lg-9">
								<input type="text" name="name" value="{{ old("name")}}" class="@error('name') is-invalid @enderror form-control" placeholder="Product name">
							</div>
						</div>
							...
							<div class="col-lg-9">
								<select id="inventory_category_id" name="inventory_category_id" class="@error('inventory_category_id') is-invalid @enderror form-control select2">
									...
									@foreach ($inventory_categories as $category)
										<option value="{{ $category->id }}"{{ ($category->id === old('inventory_category_id')) ? ' selected' : '' }}>{{ ucwords($category->name) }}</option>
									@endforeach
								</select>
							</div>
						</div>
